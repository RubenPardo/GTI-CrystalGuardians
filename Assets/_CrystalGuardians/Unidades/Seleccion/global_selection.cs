...
{
    // Script que maneja la selccion de unidades, creando un cuadro de seleccion
    ...
    private void OnGUI()
    {
        if (dragSelect == true)
        {
            var rect = Utility.GetScreenRect(p1, Input.mousePosition);
            Utility.DrawScreenRect(rect, new Color(0.8f, 0.8f, 0.95f, 0.25f));
            Utility.DrawScreenRectBorder(rect, 2, new Color(0.8f, 0.8f, 0.95f));
        }
    }
